...
if ($_SERVER["REQUEST_METHOD"] === "POST") {
  ...
  if (empty($question) || empty($answer)) {
      ...
      if (empty($message)) {
          ...
          if (preg_match('/([a-zA-Z])\1{2}/', $question) || preg_match('/([a-zA-Z])\1{2}/', $answer)) {
              $message = '<span class="consecutive-letters">Klaida:</br> Klausime arba atsakyme yra trys vienodi simboliai iš eilės.</span>';
          } elseif (strlen($question) > 25 || strlen($answer) > 25) {
              // Check word length
              $message = '<span class="word-length-error">Klaida:</br> Klausimas arba atsakymas yra per ilgi. Maksimalus leidžiamas simbolių skaičius: 25.</span>';
          } else {
              // Check user level
